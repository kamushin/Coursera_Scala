...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
  //      if (p(elem)) {
  //        filterAcc(p,left) union filterAcc(p,right) incl elem
  //      } else
  //        filterAcc(p,left) union filterAcc(p,right)
  //    }
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(elem))
  ...
  def mostRetweeted: Tweet = {
    def max(ts: TweetSet, t: Tweet): Tweet = {
      if (ts.isEmpty) t
    	...
    	val tt = ts.mostRetweeted
        if (tt.retweets > t.retweets) tt
        else t
      }
    }
  ...
  def descendingByRetweet: TweetList = {
    //    def descendingByRetweetIter(st: TweetSet): TweetList = {
    //      new Cons(st.mostRetweeted, (st remove st.mostRetweeted).descendingByRetweet)
    //    }
    //    descendingByRetweetIter(this)
    val tmp = mostRetweeted
    new Cons(tmp, (this remove tmp).descendingByRetweet)
  }
}
